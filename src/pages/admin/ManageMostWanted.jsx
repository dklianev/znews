...
export default function ManageMostWanted() {
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirm({
      title: 'Изтриване на издирвано лице',
      message: 'Лицето ще бъде изтрито безвъзвратно.',
      confirmLabel: 'Изтрий',
      variant: 'danger',
    });
    if (!confirmed) return;
  ...
  const filteredWanted = useMemo(() => {
    ...
    if (!normalizedQuery) return wanted;
    return wanted.filter((person) => (
      (person.name || '').toLowerCase().includes(normalizedQuery) ||
      (person.charge || '').toLowerCase().includes(normalizedQuery) ||
      (person.bounty || '').toLowerCase().includes(normalizedQuery)
    ));
  }, [query, wanted]);
  ...
  return (
    <div className="p-8">
      <AdminPageHeader
        title="Най-издирвани"
        description="Управление на списъка с издирвани лица"
        actions={(
      ...
      <div className="space-y-2">
        {filteredWanted.map((person, index) => (
          <div key={person.id} className="bg-white border border-gray-200 p-4 flex items-center gap-4 group">
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => { setEditing(person.id); setForm(person); }} aria-label="Редактирай издирваното лице" className="p-1.5 text-gray-400 hover:text-zn-hot"><Pencil className="w-4 h-4" /></button>
              <button onClick={() => handleDelete(person.id)} aria-label="Изтрий издирваното лице" className="p-1.5 text-gray-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
        {filteredWanted.length === 0 && (
          <AdminEmptyState
            title="Няма издирвани лица"
            description={query.trim()
              ? 'Няма записи, които да съвпадат с текущото търсене.'
              : 'Все още няма добавени лица в списъка с най-издирвани.'}
          />
        )}
      </div>
    </div>
