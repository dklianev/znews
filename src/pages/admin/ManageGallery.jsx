...
export default function ManageGallery() {
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirm({
      ...
      message: 'Снимката ще бъде изтрита безвъзвратно.',
      confirmLabel: 'Изтрий',
      variant: 'danger',
    });
    if (!confirmed) return;
  ...
  const sorted = useMemo(
    () => [...gallery].sort((a, b) => parseGalleryDate(b.date) - parseGalleryDate(a.date)),
    [gallery],
  );
  ...
  return (
    <div className="p-8">
      <AdminPageHeader
        title="Галерия"
        description={`${gallery.length} снимки`}
        actions={(
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={close}>
          <div className="bg-white max-w-lg w-full p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={close} aria-label="Затвори редактора на снимка" className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
        ...
        {filteredGallery.map(item => (
          <div key={item.id} className="group relative border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => toggleFeatured(item)} aria-label={item.featured ? 'Премахни снимката от избрани' : 'Маркирай снимката като избрана'} className="p-1 bg-white/90 text-gray-500 hover:text-amber-500" title={item.featured ? 'Премахни от избрани' : 'Маркирай като избрана'}>
                {item.featured ? <StarOff className="w-3.5 h-3.5" /> : <Star className="w-3.5 h-3.5" />}
              </button>
              <button onClick={() => openEdit(item)} aria-label="Редактирай снимката" className="p-1 bg-white/90 text-gray-500 hover:text-zn-hot" title="Редактирай">
                <Edit3 className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => handleDelete(item.id)} aria-label="Изтрий снимката" className="p-1 bg-white/90 text-gray-500 hover:text-red-500" title="Изтрий">
      ...
      {filteredGallery.length === 0 && (
        <AdminEmptyState
          ...
          description={query.trim()
            ? 'Няма снимки, които да съвпадат с текущото търсене.'
            : 'Галерията е празна. Добавете първата снимка.'}
          className="mt-6"
        />
      )}
    </div>
