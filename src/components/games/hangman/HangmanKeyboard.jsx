export default function HangmanKeyboard({ rows = [], statuses = {}, disabled = false, onGuess }) {
  return (
            ...
            {row.map((letter) => {
              ...
              return (
                <button
                  ...
                  className={`min-w-[44px] rounded-2xl border px-3 py-3 text-sm font-black uppercase tracking-[0.2em] transition-all disabled:cursor-default disabled:opacity-90 ${classes}`}
                >
                  {letter}
                </button>
              );
