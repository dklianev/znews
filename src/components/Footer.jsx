...
function FooterEditorialSkeleton() {
  return (
    <div
      className="mt-8 newspaper-page comic-panel comic-dots relative overflow-hidden min-h-[22rem] bg-white/70 dark:bg-[#201B2C]/70"
      aria-hidden="true"
    />
  );
}
...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    const loadEditorialStandards = () => {
      import('./FooterEditorialStandards.jsx')
        .then((module) => {
          if (!cancelled) {
            setEditorialStandards(() => module.default);
          }
        })
        .catch((error) => {
          console.error('Failed to load footer editorial standards.', error);
        });
    };

    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      idleHandle = window.requestIdleCallback(loadEditorialStandards, { timeout: 1200 });
    } else if (typeof window !== 'undefined') {
      timeoutHandle = window.setTimeout(loadEditorialStandards, 250);
    }
  ...
  return (
    <footer className="mt-0">
      {tipLinePromo.enabled && (
        <div className="below-fold-section-compact bg-zn-hot comic-dots border-t-8 border-b-8 border-zn-black relative overflow-hidden group">
      ...
      <div className="below-fold-section-footer comic-grid-backdrop comic-dots relative border-0 shadow-none rounded-none">
        <div className="max-w-6xl mx-auto px-4 py-10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="md:col-span-1">
              ...
              <p className="text-sm text-zn-text-muted leading-relaxed font-sans">
                Градски новини, скандали и ексклузивни разкрития от Los Santos.
              </p>
              <div className="flex items-center gap-2 mt-3 px-3 py-1.5 bg-zn-hot/10 border-2 border-zn-hot/20 inline-flex rounded-full">
            ...
            <div>
              <h4 className="font-display text-sm font-black uppercase tracking-widest text-zn-black mb-4 pb-2 border-b-2 border-zn-purple">
                Информация
              </h4>
              <ul className="space-y-2">
                {infoLinks.map((item) => (
            ...
            <div>
              <h4 className="font-display text-sm font-black uppercase tracking-widest text-zn-black mb-4 pb-2 border-b-2 border-zn-orange">
                Контакт
              </h4>
          ...
          {EditorialStandards ? <EditorialStandards /> : <FooterEditorialSkeleton />}
