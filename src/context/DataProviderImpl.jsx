...
export function DataProvider({ children }) {
  ...
  const [classifiedsMeta, setClassifiedsMeta] = useState(EMPTY_CLASSIFIEDS_META);
  ...
  const classifiedsQueryKeyRef = useRef('');
  const classifiedsRequestVersionRef = useRef(0);
  const buildClassifiedsQueryKey = useCallback((params = {}) => JSON.stringify({
    page: Math.max(1, Number.parseInt(params?.page, 10) || 1),
    limit: Math.min(100, Math.max(1, Number.parseInt(params?.limit, 10) || 25)),
    status: typeof params?.status === 'string' ? params.status : '',
    q: typeof params?.q === 'string' ? params.q : '',
  }), []);
  const refreshClassifieds = useCallback(async (params = {}) => {
    const queryKey = buildClassifiedsQueryKey(params);
    if (classifiedsLoaderRef.current?.task && classifiedsLoaderRef.current?.key === queryKey) {
      return classifiedsLoaderRef.current.task;
    }
    const requestVersion = classifiedsRequestVersionRef.current + 1;
    classifiedsRequestVersionRef.current = requestVersion;

    const task = api.classifieds.getAll(params)
      .then((data) => {
        const items = Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []);
        const nextMeta = {
          page: Number.parseInt(data?.page, 10) || Math.max(1, Number.parseInt(params?.page, 10) || 1),
          limit: Number.parseInt(data?.limit, 10) || Math.min(100, Math.max(1, Number.parseInt(params?.limit, 10) || 25)),
          total: Number.parseInt(data?.total, 10) || 0,
          totalPages: Number.parseInt(data?.totalPages, 10) || 1,
          statusCounts: data?.statusCounts && typeof data.statusCounts === 'object'
            ? {
              all: Number.parseInt(data.statusCounts.all, 10) || 0,
              awaiting_payment: Number.parseInt(data.statusCounts.awaiting_payment, 10) || 0,
              active: Number.parseInt(data.statusCounts.active, 10) || 0,
              rejected: Number.parseInt(data.statusCounts.rejected, 10) || 0,
              expired: Number.parseInt(data.statusCounts.expired, 10) || 0,
            }
            : EMPTY_CLASSIFIEDS_META.statusCounts,
          queryKey,
        };
        if (requestVersion !== classifiedsRequestVersionRef.current) {
          return { items, meta: nextMeta };
        }
        classifiedsLoadedRef.current = true;
        classifiedsQueryKeyRef.current = queryKey;
        setClassifiedsReady(true);
        setClassifieds(items);
        setClassifiedsMeta(nextMeta);
        return { items, meta: nextMeta };
      })
      .finally(() => {
        if (classifiedsLoaderRef.current?.task === task) {
          classifiedsLoaderRef.current = null;
        }
      });
    classifiedsLoaderRef.current = { key: queryKey, task };
    return task;
  }, [buildClassifiedsQueryKey]);
  const ensureClassifiedsLoaded = useCallback(async (params = {}) => {
    const canLoad = session?.role === 'admin' || permissions.some((item) => item?.role === session?.role && item?.permissions?.classifieds);
    if (!session?.token || !canLoad) return { items: [], meta: EMPTY_CLASSIFIEDS_META };
    const queryKey = buildClassifiedsQueryKey(params);
    if (classifiedsLoadedRef.current && classifiedsQueryKeyRef.current === queryKey) {
      return { items: classifieds, meta: classifiedsMeta };
    }
    try {
      return await refreshClassifieds(params);
    } catch {
      return { items: [], meta: EMPTY_CLASSIFIEDS_META };
    }
  }, [buildClassifiedsQueryKey, permissions, refreshClassifieds, session?.role, session?.token, classifieds, classifiedsMeta]);
  ...
  const adminValue = useMemo(() => ({
    ...
    classifieds, classifiedsMeta, classifiedsReady, refreshClassifieds, ensureClassifiedsLoaded, approveClassified, rejectClassified, deleteClassified, bumpClassified, renewClassified,
    resetAll,
  }), [
    articleRevisions, loadArticleRevisions, autosaveArticleRevision, restoreArticleRevision,
    heroSettingsRevisions, loadHeroSettingsRevisions, restoreHeroSettingsRevision,
