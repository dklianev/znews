...
const dbPath = path.join(__dirname, 'data', 'zemun_news.db');
const db = new sqlite3.Database(dbPath);

const title = 'Това не е слух. Това е предупреждение.';
const excerpt = 'Градът ни се управлява не от закона, а от хора с костюми и връзки в сенките. Raphael Ragucci всъщност дърпа конците на престъпна структура.';
...
db.run(
    `INSERT INTO articles (title, excerpt, content, category, authorId, date, readTime, status, featured, breaking) 
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [title, excerpt, content, 'news', 'system', dateStr, readTime, 'published', 1, 1],
    function (err) {
        if (err) {
            console.error('Error inserting:', err);
            process.exit(1);
        } else {
            console.log('Successfully inserted anonymous tip article! ID:', this.lastID);
            db.close();
        }
    }
);
