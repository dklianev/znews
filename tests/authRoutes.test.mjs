...
describe('authRoutes', () => {
  it('logs in bcrypt users and rotates refresh tokens', async () => {
  ...
  it('rejects plaintext passwords instead of auto-upgrading them', async () => {
    const app = createMockApp();
  ...
  it('treats empty refresh and empty credentials as no-op auth failures', async () => {
    const app = createMockApp();
    let cleared = 0;
