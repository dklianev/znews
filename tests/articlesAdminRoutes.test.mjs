...
describe('articlesAdminRoutes', () => {
  it('builds paginated admin list filters for category and text search', async () => {
  ...
  it('clamps out-of-range page requests to the last available admin page', async () => {
    const router = createMockRouter();
  ...
  it('returns admin article metadata grouped by category and tag popularity', async () => {
    const router = createMockRouter();
  ...
  it('builds related article lookups from deduped ids and trimmed query text', async () => {
    const router = createMockRouter();
    const calls = [];
    registerArticlesAdminRoutes(router, createDeps({
  ...
  it('short-circuits related lookups when no candidate ids remain after exclusion', async () => {
    const router = createMockRouter();
